      ...
      {categories.map((item) => (
          <div 
            ...
            <div  className=" p-8 rounded-2xl cursor-pointer bg-primary">
              <img 
              src={item.image} alt="categoryImg " height={155} 
              width={155} className="object-cover h-32"/>
            </div>
            <h4 
            className={`mt-6 medium-18 ${category  === item.name ? 
            "border-b-4 border-secondary" : "border-b-4 border-white"}`}>
              {item.name}
            </h4>
          </div>
        ))}
